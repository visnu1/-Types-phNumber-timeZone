...
function getCountryInfo(object) {
  ...
  var countryIndex: any = false;

  for (let i = 0; i < countryCodes.length; i++) {
    if (code === countryCodes[i]['code']) {
      country_info = countryCodes[i];
...
function checkDst(object) {
  ...
  if (start_offset) {
    //Only affects Jerusalem and Jordan

    var day = startDate.getDate();
    day = day + start_offset;
...
function formatTime(hour, mins, offset, military) {
  ...
  if (partialOffset > 0) {
    var offsetMinutes = partialOffset * 60;
